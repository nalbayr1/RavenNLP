...
const PlayerList: React.FC<PlayerListProps> = ({ searchQuery = '', players: initialPlayers }) => {
  ...
  // State for managing the visibility of NotesModal
  const [notesModalPlayer, setNotesModalPlayer] = useState<Player | null>(null);
    ...
    if (!initialPlayers) {
      const fetchPlayers = async () => {
        try {
          ...
          setPlayers(data);
          // Initialize bio visibility for each player
          setBioVisible(
  ...
  // Functions to open and close the NotesModal
  const openNotesModal = (player: Player) => {
  ...
  return (
    ...
    >
      ...
      {/* Render the NotesModal when a player is selected */}
      {notesModalPlayer && (
        <NotesModal player={notesModalPlayer} onClose={closeNotesModal} />
      )}
    </div>
  );
};
